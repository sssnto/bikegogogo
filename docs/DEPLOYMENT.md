...
<string>BikeGoGo 需要相机用于骑行小队实时通信中的视频交流；未开启相关功能时不会拍摄或保存内容。</string>
...
当前 App 已指向公网服务 `https://bikegogogo-server.sssnto.cn:8443`。LiveKit identity
由后端账户决定，可以用好友房间或自己创建的小队进行双机测试：

1. 两台 iPhone 都安装当前版本。
2. 互相通过好友码发送并接受好友申请。
3. A 在“语音”页选择好友并点击“发起语音”；B 前台应出现接听页，后台或锁屏时应
   收到带声音的通知。
4. B 点击“接听”，两边成员列表应分别显示“麦克风已发送”和“语音已接收”。
5. 两边轮流说话和静音，确认双向都能听见，并核对页面显示的音频输出设备。
6. 创建小队并邀请另一位好友，重复发起、接听和取消流程。
7. 通话中测试锁屏、切后台，并切换 Wi-Fi 和蜂窝网络；界面应短暂显示“正在重连”后
   恢复。
8. 完成一条真实骑行，在“历史”页确认出现“已同步”；另一台同账号设备下拉刷新后应
   看到同一条路线和指标。

语音邀请依赖新版后端的 `/v1/voice/invitations` 接口和 APNs。双机测试前必须先更新
NAS 镜像，再安装包含新客户端的构建；新客户端连接旧后端会提示服务器无法处理语音请求。
