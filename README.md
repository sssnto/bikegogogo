# BikeGoGo

BikeGoGo 是一个面向自行车骑行爱好者的 iOS + Apple Watch 应用原型，核心目标是解决两件事：

1. 骑行好友在双方同意后，可以进入稳定的实时语音房间沟通。
2. 记录骑行路线、速度、距离、时长、心率等数据，并与 Apple Watch 和 Apple 健康联动。

当前仓库处于 MVP 真机联调阶段，包含：

- 产品设计文档。
- 技术设计文档。
- 开发计划。
- 部署文档。
- 可通过 `swift build` 校验的核心 Swift 包。
- 可直接打开运行的 iOS + watchOS Xcode 工程。
- CoreLocation 路线记录、本地恢复、历史记录和 GPX 导出。
- Apple Watch HealthKit workout 与双向状态、心率同步；户外训练关联 GPS 路线，
  并写入爬升、速度、暂停事件和每 5 公里圈段。
- iPhone 开始骑行时自动唤醒 Watch 户外单车训练，无需再到系统“体能训练”重复开始。
- 结束骑行后自动回读苹果健身中的路线、距离、心率、热量、踏频和功率，并与手机
  实时记录合并；历史页也支持手动导入最近一年的户外单车训练。
- LiveKit Swift SDK 真实语音房间、静音、成员状态和自动重连。
- Sign in with Apple、访客账户原地绑定和 Keychain 会话。
- 好友申请、小队创建/邀请/成员管理和好友或小队语音。
- 骑行中可按小队开启临时位置共享，并在地图查看队友位置与速度。
- 骑行中可向小队发送带当前位置的 SOS，成员通过 APNs 接收并打开地图查看。
- 小队位置共享期间显示队友距离，并提供掉队和位置中断的一次性本地提醒。
- 小队创建者可设置带名称的 6 小时集合点，成员可查看剩余距离、到达状态并启动 Apple 地图骑行导航。
- 骑手进入集合点 100 米范围并保持 10 秒后收到一次到达提醒，离开 180 米后可重新判定。
- 集合点根据当前或本次平均骑行速度显示预计到达时间，小队面板汇总全队到达进度。
- 位置共享按移动状态自适应上报，暂停骑行时停止 GPS 更新以降低户外耗电。
- 实时共享位置与历史轨迹滤波独立，定位纠正后会优先更新队友、SOS 和集合点坐标。
- 中国大陆地图显示会把 WGS-84 骑行坐标转换为高德底图使用的 GCJ-02；统计、云同步
  和 GPX 导出继续保存 WGS-84 原始坐标。
- 骑行地图可一键呼叫当前小队，并在地图内静音或结束语音。
- 户外语音启用回声消除、降噪、自动增益和高通滤波，并使用 Opus RED 冗余及移动
  网络优先级降低风噪、音量不足和弱网断续。
- LiveKit Cloud 语音额外启用客户端 Krisp 增强降噪，重点抑制风声、车流和持续环境噪声。
- 可选历史骑行作为蓝色虚线参考路线，持续偏航后在本机提醒。
- 已登录账户鉴权的 LiveKit 短期 token。
- 完成骑行后自动上传、跨设备历史同步、手动刷新和云端删除。
- 账户权限状态、隐私政策、个人数据 JSON 导出和应用内永久注销。
- APNs 好友申请、通过和小队邀请通知。
- PostgreSQL 16 主存储、旧 JSON 自动迁移和回滚镜像。

## 推荐开发环境

- macOS 15 或更新版本。
- Xcode 26.3 或更新版本。
- iOS 17+。
- watchOS 10+。
- Apple Developer Program 账号，后续真机、HealthKit、Push、TestFlight 都会用到。

## 目录

```text
.
├── Apps
│   ├── iOS
│   │   └── BikeGoGo
│   └── watchOS
│       └── BikeGoGoWatch
├── BikeGoGo
│   └── BikeGoGo.xcodeproj
├── Sources
│   └── BikeGoGoCore
├── Tests
│   └── BikeGoGoCoreTests
├── docs
└── server
```

## 本地校验

核心模型和统计计算可以先独立构建：

```bash
swift build
swift test
```

完整 App 已有 Xcode 工程：

```bash
open BikeGoGo/BikeGoGo.xcodeproj
```

在 Xcode 中选择 `BikeGoGo` 运行 iPhone App，选择 `BikeGoGoWatch` 运行 Watch App。首次打开会自动解析 LiveKit Swift Package。详细步骤见 [部署文档](docs/DEPLOYMENT.md)。

命令行构建：

```bash
xcodebuild -project BikeGoGo/BikeGoGo.xcodeproj \
  -scheme BikeGoGo \
  -destination 'generic/platform=iOS Simulator' \
  CODE_SIGNING_ALLOWED=NO build

xcodebuild -project BikeGoGo/BikeGoGo.xcodeproj \
  -scheme BikeGoGoWatch \
  -destination 'generic/platform=watchOS Simulator' \
  CODE_SIGNING_ALLOWED=NO build
```

## 文档入口

- [产品设计](docs/PRODUCT_DESIGN.md)
- [技术设计](docs/TECHNICAL_DESIGN.md)
- [开发计划](docs/DEVELOPMENT_PLAN.md)
- [API 设计](docs/API_DESIGN.md)
- [部署文档](docs/DEPLOYMENT.md)
- [中间件准备清单](docs/MIDDLEWARE_PREP.md)
- [NAS Docker 部署](docs/NAS_DOCKER_DEPLOYMENT.md)
- [APNs 私钥申请与保管](docs/APNS_SETUP.md)
- [运营管理后台设计](docs/ADMIN_SYSTEM_DESIGN.md)
- [运营管理后台部署](docs/ADMIN_DEPLOYMENT.md)
